...
$file  = "process_data/data/20130129 Stage2 All - trimmed for aggregations.csv"; //input file
      ...
      foreach ($row1 as $key=>$value) { //e.g. $row1[0] = projectid
        if (in_array($value,$allowed_columns)) {
          ...
          if ($value == "Created_date") {
            $data[(int)$key] = substr($data[(int)$key],0,4);
          }
          if ($value == "applicant_numberbenefit") {
            $applicant_numberbenefit = $data[(int)$key];
            continue;
          }
          if ($value == "NT_funding") {
            $data[(int)$key] = $data[(int)$key] . "," . $applicant_numberbenefit;
            unset($applicant_numberbenefit);
          }
          /*if ($value == "Start_date") {
            $start_date = $data[(int)$key];
            $year = date("Y",strtotime($start_date));
            echo $year;
          }
          if ($value == "Organisation_type") {
            $org_type = $data[(int)$key];
            echo $org_type;
          }
          if(isset($year) && isset($org_type)) {
            $this_row_to_array["years"][$year][] = $org_type;
            unset($org_type);
            unset($year);
            unset($start_date);
          }*/
...
//print_r($this_row_to_array["years"]); die;
$values = array_count_values($this_row_to_array["Status"]);
$stage2_years = array_count_values($this_row_to_array["Created_date"]);
ksort($stage2_years);
